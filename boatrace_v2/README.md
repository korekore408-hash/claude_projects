# boatrace_v2 — 予想・EV・回収率パイプライン v2
...
残課題（フェーズ2）: T10 特徴量再選定（モデルはv1を利用中）、UI（today.html 相当）、
展示・結果取得のv2化（現状は結果=K-file、展示は未対応）。

## 構成

```
config.py       共通設定（並列度・閾値・パス。v1データはここから参照）
...
ev_picks.py     当日EVピック（較正確率×スナップショット・鮮度表示）
server.py       認証付き配信サーバ
test_v2.py      ユニットテスト（ネットワーク不要・17件）
data/           v2 の出力（スナップショット・発走時刻・較正曲線）
```
...
# 3) オッズ収集（当日。締切5分前ブースト付きで常駐）
python scheduler.py                 # または CI から: python scheduler.py --once
...
# 6) 配信（LAN公開はトークン必須）
SERVE_TOKEN=xxxx python server.py --bind 0.0.0.0
```
