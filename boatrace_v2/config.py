...
APP_TITLE = "Nishio-boooto"   # アプリ名（変更はここ1箇所）
...
# v1 が生成する特徴量CSV（T10 特徴量再選定の入力）
V1_HIST = os.path.join(V1_DIR, "features_player_history.csv")
V1_REL = os.path.join(V1_DIR, "features_race_relative.csv")

BEFORE_DIR = os.path.join(DATA_DIR, "before")           # 展示・結果（v2）
V1_BEFORE_DIR = os.path.join(V1_DATA, "before")         # v1 形式（フォールバック読込）
SELECTED_FEATURES_PATH = os.path.join(DATA_DIR, "selected_features.json")
...
def ensure_dirs():
    for d in (DATA_DIR, ODDS_SNAP_DIR, START_TIMES_DIR, WEB_DIR, BEFORE_DIR):
