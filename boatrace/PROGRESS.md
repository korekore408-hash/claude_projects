...
- フェーズ2（2026-07-02 UI 完了）: アプリ名「西をポーターズ」（config.APP_TITLE で変更可）。
  `app.py` 1コマンドで「オッズ収集＋today.html 生成＋配信」が起動
  （report.py / ev_picks.compute_picks。ユニットテスト21件）。
  残: T10 特徴量再選定、展示取得のv2化。v1 は特徴量・予測CSVの生成元として当面併用する。
