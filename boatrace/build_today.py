...
def _fuku_std(s):
    """2連複「標準目のみ」（サイト本体 fukuStd と同一）: PL確率上位15ペアのうち、
    想定オッズ（=1÷PL確率）が標準帯 COMBO_ODDS_CUTS["ex"]=[3.5,5.5) に入る組を全部拾う。
    2026-09 の買い目UI変更（本命1本を廃止＝標準目のみ）に回収率集計を合わせるため。"""
    lo, hi = COMBO_ODDS_CUTS["ex"]
    out = []
    for pr in _pf_topk(s, 15):
        p = _pf_prob(s, pr)
        if p <= 0:
            continue
        od = 1.0 / p
...
def daily_recovery(rel, pred, model_map, api_map, hon_canon, payout, base, ndays=30):
    ...
    ★買い目＝サイト本体／上部サマリー(daySummary)と同一ポリシー（順位付け＝学習モデル、
      荒れ度・点数＝API本命確率）。各日の 2連複／3連単／穴目 と、直近窓のレース荒れ度帯別
    ...
    for rid, rc in races.items():
        ...
        D = day[rc["d"]]
        bi = 0 if hon >= 0.65 else (2 if hon < 0.45 else 1)   # レース荒れ度帯 本命/標準/波乱
        D["nr"][bi] += 1
    ...
    for d in days:
        for k in keys:
            for i in range(4):
                tot[k][i] += day[d][k][i]
    # レース荒れ度帯別（本命/標準/波乱）＝直近窓での券種別 累計回収率グラフ用。
    bands = []
    for bi in range(3):
        ex = [0, 0, 0, 0]
        tri = [0, 0, 0, 0]
...
def game_ledger_mix(rel, pred, model_map, hon_canon, payout, start_date,
    ...
    ★ルール（2026-09-01〜／穴帯版・鉄板版の旧方式は廃止）:
      ...
      - 各レースの買い目は2連複 上位3点（2026-09-13〜・的中率重視。旧「3連単のみ」は廃止）＝
        PL確率上位3ペアにレース予算を全額・確率比例配分。実配当で精算・フライングは返還。
        （検証: 各日上位10Rで的中79%・回収84%＝旧3連単のみ 的中41%/回収80% より「当たる」）
...
function pvToEx(pv){
  ...
  let any=false;
  // boats は openapi の 0始まり配列（艇番は各要素の racer_boat_number）。
  // 位置で引くと艇番とズレて6号艇が欠落するため、必ず racer_boat_number で振り分ける。
  // オブジェクト形式（"1".."6"）にも後方互換で対応。
  const list = Array.isArray(bo) ? bo : (bo ? Object.values(bo) : []);
  list.forEach(b=>{
    if(!b)return;
    const w = +(b.racer_boat_number); if(!(w>=1&&w<=6))return;
